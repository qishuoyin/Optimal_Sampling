...
parent_parent_parent_dir = dirname(parent_parent_dir)

# set seeds
set.seed(2024) 

# import necessary sources
fun_dir = paste(c(parent_parent_parent_dir, "R"), collapse="/")
...
I_vec = 200 # number of units
K_vec = 10 # number of outcomes
Gamma_vec = c(1, 1.25, 1.5, 1.75, 2) # design sensitivity
xi_vec = seq(from = 0.01, to = 0.99, by = 0.01) # analysis sample fraction
method = "select"


test_result_dir = paste(c(parent_dir, "output/output_test"), collapse="/")
final_result_dir = paste(c(parent_dir, "output/output_evaluation"), collapse="/")
    ...
    for(i3 in 1:length(Gamma_vec)) {
      ...
      for(i4 in 1:length(xi_vec)) {
        ...
        for(t in 1:test_sim) {
          ...
          data_path = paste(c(paste(c(parent_dir, "data/data_simulation"), collapse="/"), data_name), collapse="/")
