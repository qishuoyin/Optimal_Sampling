...
# Simulation: "select" method - job array version

# pass parameters from txt file
args <- commandArgs(trailingOnly = TRUE)
param1 <- as.numeric(args[1])
param2 <- as.numeric(args[2])

# Your analysis code using the parameters
print(paste("Parameter 1:", param1))
print(paste("Parameter 2:", param2))
...
# Conduct simulation by the optimal sample split method - "select"
...
for(i3 in 1:length(Gamma_vec)) {
  ...
  for(i4 in 1:length(xi_vec)) {
    ...
    planning_file_name = paste(c(paste(c("test_planning", file_info), collapse="_"), "csv"), collapse=".")
    analysis_file_name = paste(c(paste(c("test_analysis", file_info), collapse="_"), "csv"), collapse=".")
...
result_file_name = paste(c(paste(c("final_result", "outcome", K, "I", I, "method", method), collapse="_"), "csv"), collapse=".")
