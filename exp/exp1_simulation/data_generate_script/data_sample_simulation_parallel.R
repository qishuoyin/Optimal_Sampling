...
foreach(i1 = 1:length(K_vec), .packages="foreach") %dopar% {
  
  K = K_vec[i1]
  
  foreach(i2 = 1:length(Gamma_vec), .packages="foreach") %dopar% {
    
    Gamma = Gamma_vec[i2]
    
    foreach(i3 = 1:length(I_vec), .packages="foreach") %dopar% {
      
      I = I_vec[i3]
      
      foreach(t = 1:test_sim, .packages="foreach") %dopar% {
